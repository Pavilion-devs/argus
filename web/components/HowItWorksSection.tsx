      ...
      <div className="flex flex-col min-h-[55vh] overflow-x-hidden xl:flex-row xl:overflow-hidden xl:px-20 bg-[#000000] w-full pt-0 pb-20 relative">
        <div className="reveal-element xl:w-[28%] flex-shrink-0 flex flex-col xl:p-10 transition-all duration-500 z-10 xl:shadow-[10px_0_30px_rgba(0,0,0,0.5)] overflow-hidden bg-[#2563eb] w-full pt-8 pr-8 pb-8 pl-8 relative shadow-2xl border border-[#27272a]">
          ...
          <div className="mt-12 xl:mt-16 space-y-4 relative z-10">
            <h2 className="text-2xl tracking-tight text-white font-medium font-geist">Alert → Plan</h2>
            <p className="text-white/85 text-sm leading-relaxed pr-4 font-extralight font-geist">Argus takes the notable, recalls its own past cases and active blocklist, and declares the leading hypotheses it will test — before it runs a single query.</p>
          </div>
          <div className="mt-10 xl:mt-12 rounded-xl overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.3)] group relative">
            ...
            <ParticleCanvas />
            <img alt="Security operations console" className="w-full h-56 xl:h-64 object-cover object-center grayscale opacity-80 transition-all duration-1000 ease-out group-hover:scale-105 group-hover:grayscale-0 group-hover:opacity-100" src="https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?q=80&w=1200&auto=format&fit=crop" />
          </div>
          <div className="mt-12 xl:mt-auto pt-8 flex flex-col gap-2 relative z-10">
            <Icon className="text-3xl text-white/90" icon="solar:checklist-minimalistic-linear" strokeWidth="1.5" />
            <span className="text-lg font-medium tracking-wide font-geist">planning</span>
          </div>
        </div>
        <div className="stagger-container flex flex-row xl:flex-row w-full xl:w-auto divide-x divide-[#27272a] border-y xl:border-y-0 xl:border-y border-y-[#27272a] xl:border-r border-[#27272a]">
          <div className="stagger-item bg-[#18181b] flex-1 flex-shrink-0 flex flex-col justify-between p-6 xl:p-8 hover:bg-[#1e1e22] transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] cursor-pointer group xl:w-[140px] xl:flex-none">
            <div>
              ...
              <div className="mt-6 overflow-hidden transition-all duration-500 max-h-0 opacity-0">
                <p className="text-zinc-400 text-sm font-extralight leading-relaxed font-geist whitespace-normal">Writes SPL on the fly and runs it through the Splunk MCP Server — the only way Argus ever touches Splunk.</p>
              </div>
            </div>
            <div className="flex items-center justify-between mt-12 xl:mt-auto pt-8">
              <span className="text-sm tracking-wide transition-colors duration-300 font-extralight font-geist text-zinc-500 group-hover:text-zinc-300">querying</span>
          ...
          <div className="stagger-item bg-[#18181b] flex-1 flex-shrink-0 flex flex-col justify-between p-6 xl:p-8 hover:bg-[#1e1e22] transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] cursor-pointer group xl:w-[140px] xl:flex-none">
            <div>
              ...
              <div className="mt-6 overflow-hidden transition-all duration-500 max-h-0 opacity-0">
                <p className="text-zinc-400 text-sm font-extralight leading-relaxed font-geist whitespace-normal">Reads the result rows, confirms or refutes each hypothesis, and decides the next query. No hardcoded paths.</p>
              </div>
            </div>
            <div className="flex items-center justify-between mt-12 xl:mt-auto pt-8">
              <span className="text-sm tracking-wide transition-colors duration-300 font-extralight font-geist text-zinc-500 group-hover:text-zinc-300">pivoting</span>
              <Icon className="text-xl text-zinc-500 transition-all duration-500 opacity-100 group-hover:text-zinc-300" icon="solar:add-circle-linear" />
            </div>
          </div>
          <div className="stagger-item bg-[#18181b] flex-1 flex-shrink-0 flex flex-col justify-between p-6 xl:p-8 hover:bg-[#1e1e22] transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] cursor-pointer group xl:w-[140px] xl:flex-none">
            <div>
              ...
              <div className="mt-6 overflow-hidden transition-all duration-500 max-h-0 opacity-0">
                <p className="text-zinc-400 text-sm font-extralight leading-relaxed font-geist whitespace-normal">Files a grounded report, contains the threat through a gated response, and installs a detection so it can't recur.</p>
              </div>
            </div>
            <div className="flex items-center justify-between mt-12 xl:mt-auto pt-8">
              <span className="text-sm tracking-wide transition-colors duration-300 font-extralight font-geist text-zinc-500 group-hover:text-zinc-300">hardening</span>
              <Icon className="text-xl text-zinc-500 transition-all duration-500 opacity-100 group-hover:text-zinc-300" icon="solar:add-circle-linear" />
            </div>
          </div>
        </div>
        <div className="bg-[#18181b] border-y border-r border-[#27272a] flex-1 flex flex-col p-8 xl:p-14 2xl:p-20 relative overflow-hidden transition-all duration-500">
          <div className="reveal-element flex items-center gap-4 text-xs font-normal text-zinc-400 w-full max-w-4xl font-geist">
            <span className="whitespace-nowrap">Plan · Act · Observe</span>
            <div className="flex-1 h-px bg-[#27272a]"></div>
            <span className="whitespace-nowrap">MCP-native</span>
          </div>
          <div className="mt-16 xl:mt-24 max-w-3xl">
            <h1 className="flex flex-wrap gap-x-[0.25em] gap-y-[0.1em] text-4xl md:text-5xl lg:text-6xl font-medium tracking-tight text-zinc-100 leading-[1.1] font-geist">
              <MaskedText text="Plan, act, observe — then prove every claim" />
            </h1>
            <p className="reveal-element mt-8 text-zinc-400 text-base md:text-lg leading-relaxed max-w-2xl font-extralight font-geist">The loop keeps running until the verdict is grounded. If a first pass hedges, a confidence-gated continuation pursues the decisive pivot before Argus commits — and in multi-agent mode, four specialists investigate in parallel and a synthesizer correlates them into one narrative.</p>
          </div>
